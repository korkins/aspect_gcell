...
//
	...
	int
		code, molec_id, iline, iline0, nlines, niso, iso_ix, nnu, inu, inu1, inu2; // inu_lo, inu_hi;
	double
		time_start, time_end,
	...
	char 
		k_or_tau, // print X-section or tau_abs
...
//  Dafault scenario: AO2
	...
	nu_usr_max = 13160.0;
	dnu = 0.01;
//	strcpy(k_or_tau, "k"); // print k (cm2/molec)
	k_or_tau = 'k';
...
//
	...
	else
	{
		...
		{ // read input file: data in the first line
			fscanf(pFile, "%d %lf %lf %lf %lf %lf %lf %c", &molec_id,
				&nu_usr_min, &nu_usr_max, &dnu,
					&l_cm, &T_kelvin, &p_atm, &k_or_tau);
			fclose(pFile);
			printf("\n closed: %s", argv[1]);
			flag_input_file = true;
		} // if pFile is missing
...
//		if (strcmp(k_or_tau, "k") == 0)
		if (k_or_tau == 'k')
		{
			printf("\n output paramter: X-section k(nu) (cm2/molec)");
			for (inu = 0; inu < nnu; inu++)
				fprintf(pFile, "%9.3f  %12.6e\n", nu[inu], k_abs[inu]);
		}
		else
		{
			printf("\n output paramter: optical thickness tau_abs(nu)");
